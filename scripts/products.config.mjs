...
export const products = [
    ...
    units: [
      {
        ...
        // 通常の検索キーワードのままだと、同じ価格帯にある「4箱セット」等の
        // 商品が多く、6箱セットが埋もれやすいため、検索キーワード自体を
        // 絞り込む。「ワンデー」を含めると、「1dayPure」表記のみで
        // 「ワンデー」の文字を含まないショップが検索結果から漏れてしまう
        // ため、「うるおいプラス」を軸にした、より広いキーワードにしている。
        hintedKeyword: "うるおいプラス 乱視用 6箱",
        introHtml: "",
        matches(name) {
  ...
  // ------------------------------------------------------------------
  // 9. 新しい商品を追加する場合は、ここに同じ形式でオブジェクトを追加する。
  // ------------------------------------------------------------------
];
