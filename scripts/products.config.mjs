...
export const products = [
  ...
  // ------------------------------------------------------------------
  // 16. シードワンデーピュアマルチステージ 遠近両用
  // ------------------------------------------------------------------
  {
    id: "seed-multistage-saiyasu",
    slug: "seed-multistage-saiyasu",
    outputDir: "docs-seed-multistage",
    siteName: "シードワンデーピュアマルチステージ 遠近両用最安値通販価格情報",
    theme: { accent: "#6B8E4E", gold: "#B8892B" }, // オリーブグリーン系(他16サイトと見分けやすい配色)
    historyUnitKey: "box6", // 価格推移グラフで記録する比較単位（6箱が実勢価格でも最安のため）
    searchKeyword: "シードワンデーピュアマルチステージ",
    metaDescription:
      "シードワンデーピュアマルチステージ 遠近両用の楽天市場・Yahoo!ショッピングの価格を毎日チェックし、6箱・4箱・2箱・1箱それぞれの最安値トップ5を掲載しています。",
    subtitle:
      "「6箱」「4箱」「2箱」「1箱」それぞれの単位を基準に、楽天市場・Yahoo!ショッピングの価格を毎日チェックし、それぞれのショップ別最安値トップ5を掲載しています。",
    productSchemaName: "シードワンデーピュアマルチステージ 遠近両用 6箱セット（192枚）",
    brand: "シード(国産)", // トップページのブランド別グルーピングに使用
    brandKey: "seed",
    shortName: "シードワンデーピュアマルチステージ 遠近両用", // トップページの商品カード・フッターで使う短い正式名称
    ...
    // この商品には90枚パック等の設定が無く、32枚入りの1/2/4/6箱のみで
    // 展開されている。
    units: [
      {
        key: "box6",
        ...
        totalLenses: 192,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "シードワンデーピュアマルチステージ 遠近両用 6箱セット(192枚)",
        // 参考価格(6箱13,240円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 10500, max: 16500 },
        // 「シードワンデーピュアマルチステージ」と「シードピュアマルチ
        // ステージ」(「ワンデー」の表記が無いショップがある)の両方で検索する
        hintedKeywords: ["シードワンデーピュアマルチステージ", "シードピュアマルチステージ"],
        introHtml: "",
        matches(name) {
          return isBoxCount(name, 6) || /192枚/.test(name.replace(/\s/g, ""));
        },
      },
      {
        ...
        totalLenses: 128,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "シードワンデーピュアマルチステージ 遠近両用 4箱セット(128枚)",
        // 参考価格(4箱8,760円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 7000, max: 11000 },
        // 「シードワンデーピュアマルチステージ」と「シードピュアマルチ
        // ステージ」(「ワンデー」の表記が無いショップがある)の両方で検索する
        hintedKeywords: ["シードワンデーピュアマルチステージ", "シードピュアマルチステージ"],
    ...
    <p>
      ...
      },
      {
        ...
        totalLenses: 64,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "シードワンデーピュアマルチステージ 遠近両用 2箱セット(64枚)",
        // 参考価格(2箱4,379円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 3500, max: 5500 },
        // 「シードワンデーピュアマルチステージ」と「シードピュアマルチ
        // ステージ」(「ワンデー」の表記が無いショップがある)の両方で検索する
        hintedKeywords: ["シードワンデーピュアマルチステージ", "シードピュアマルチステージ"],
        introHtml: `    <h2 class="section-heading">2箱(単品)でも比較したい方へ</h2>
    <p>
      ...
      },
      {
        ...
        totalLenses: 32,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "シードワンデーピュアマルチステージ 遠近両用 1箱(32枚)",
        // 参考価格(1箱2,420円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 1900, max: 3000 },
        // 「シードワンデーピュアマルチステージ」と「シードピュアマルチ
        // ステージ」(「ワンデー」の表記が無いショップがある)の両方で検索する
        hintedKeywords: ["シードワンデーピュアマルチステージ", "シードピュアマルチステージ"],
        introHtml: `    <h2 class="section-heading">1箱(単品)でも比較したい方へ</h2>
    ...
    productIntroHtml: `    <h2 class="section-heading">6箱(192枚)あたりの価格を中心に比較しています</h2>
    <p>
      シードワンデーピュアマルチステージ 遠近両用は、1箱(32枚)を何箱まとめて
      購入するかによって、1枚あたりの単価が変わります。1枚あたりの単価が
      もっとも下がりやすいのは、6箱(192枚・約6ヶ月分)を購入した場合です。
      そのため当サイトでは、6箱あたりの実勢価格を中心に比較しつつ、
      4箱・2箱・1箱で見つかった場合も、それぞれ別枠であわせて掲載しています。
    </p>
    <p class="note">
      ※ 販売単位や内容量は、時期やショップにより変更・終了している場合があります。
    ...
    productInfoHeading: "シードワンデーピュアマルチステージ 遠近両用とは",
    productInfoHtml: `        <p>
          シードワンデーピュアマルチステージ 遠近両用は、国内の医療機器
          メーカーである株式会社シードが展開する、純国産の遠近両用
          1日使い捨てタイプのコンタクトレンズです。レンズの中に遠くと
          近くそれぞれの度数が配置されており、1枚のレンズで遠近どちらも
          見られる設計とされています。
        </p>
        <h3>自然な見え方を追求した光学設計</h3>
        <p>
          通常の1日使い捨てレンズからのスムーズな移行が可能とされる光学
          設計のため、初めて遠近両用コンタクトレンズを使う方にもおすすめ
          とされています。加入度数は2種類([A]+0.75、[B]+1.50)の展開です。
        </p>
        <h3>うるおいを保つ設計</h3>
        <p>
          水分を引き寄せてとどめる力が強いシード独自の両性イオン素材
          「SIB」に加え、保存液にはアルギン酸などのうるおい成分を配合。
          UVカット機能も備えています(UVカット機能はあくまで補助的な
          ものであり、サングラス等の代わりにはなりませんのでご注意
          ください)。
        </p>
        <h3>こんな方におすすめ</h3>
        <ul>
          <li>初めて遠近両用コンタクトレンズを使う方</li>
          <li>装用中のうるおいを重視したい方</li>
          <li>国内メーカーのレンズを使いたい方</li>
        </ul>
        <p class="note">
          ※ 度数・加入度数・カーブなどの詳細仕様は変更される場合があります。
          遠近両用レンズは見え方の感じ方に個人差が大きいため、
          ご購入前に必ず眼科での検査・処方をふまえてご確認ください。
          本ページは購入の最終判断材料としてではなく、比較検討のための
          参考情報としてご活用ください。
  ...
  // ------------------------------------------------------------------
  {
    id: "biotrue-oneday-saiyasu",
    slug: "biotrue-oneday-saiyasu",
    outputDir: "docs-biotrue-oneday",
    siteName: "バイオトゥルーワンデー最安値通販価格情報",
    theme: { accent: "#8C4A6B", gold: "#B8892B" }, // ワイン系(他17サイトと見分けやすい配色)
    historyUnitKey: "pack90x2", // 価格推移グラフで記録する比較単位（90枚バリューパック2箱が1枚あたりでも最安のため）
    searchKeyword: "バイオトゥルーワンデー",
    metaDescription:
      "バイオトゥルーワンデーの楽天市場・Yahoo!ショッピングの価格を毎日チェックし、90枚バリューパック2箱・90枚バリューパック1箱・2箱・1箱それぞれの最安値トップ5を掲載しています。",
    subtitle:
      "「90枚バリューパック2箱」「90枚バリューパック1箱」「2箱」「1箱」それぞれの単位を基準に、楽天市場・Yahoo!ショッピングの価格を毎日チェックし、それぞれのショップ別最安値トップ5を掲載しています。",
    productSchemaName: "バイオトゥルーワンデー 90枚バリューパック×2箱セット（180枚）",
    brandName: "Bausch+Lomb", // JSON-LD(構造化データ)のbrand.nameに使用
    brand: "ボシュロム", // トップページのブランド別グルーピングに使用
    brandKey: "bausch",
    shortName: "バイオトゥルーワンデー", // トップページの商品カード・フッターで使う短い正式名称

    // トップページのスペック比較表に使用(メーカー公式・販売店情報を確認のうえ設定)
    specs: {
      material: "ハイパージェル(独自素材)",
      water: 78, // 含水率(%)
      dk: 42, // 酸素透過率(Dk/t、参考値)
      uv: true, // UVカットの有無
      origin: "海外",
      type: "標準（近視）",
    },

    /** 商品名が「バイオトゥルーワンデー」（マルチフォーカル・乱視用等の別ラインではない）であることを確認する */
    isCorrectProduct(name) {
      if (!name) return false;
      const n = name.replace(/\s/g, "");
      if (/(マルチフォーカル|遠近両用|乱視用|トーリック|toric)/i.test(n)) return false;
      if (!/バイオトゥルー/.test(n)) return false;
      return /ワンデー/.test(n) || /1day/i.test(n);
    },

    // 標準サイズ(30枚入り)と90枚バリューパックで内容量(1箱あたりの
    // 枚数)が異なる点に注意。それぞれの箱数(1箱/2箱)とあわせて、
    // 計4つの比較単位を設定する。
    units: [
      {
        ...
        totalLenses: 180,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデー 90枚バリューパック×2箱セット(180枚)",
        // 参考価格(90枚バリューパック2箱9,255円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 7400, max: 11500 },
        hintedKeyword: "バイオトゥルーワンデー 90枚 2箱",
        introHtml: "",
        /** 「90枚バリューパックを2箱」らしきものだけを判定する */
        matches(name) {
          if (!name) return false;
      ...
      },
      {
        ...
        totalLenses: 90,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデー 90枚バリューパック1箱",
        // 参考価格(90枚バリューパック1箱5,998円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 4800, max: 7500 },
        hintedKeyword: "バイオトゥルーワンデー 90枚パック",
        introHtml: `    <h2 class="section-heading">90枚バリューパック(単品)でも比較したい方へ</h2>
    <p>
      90枚バリューパックを1箱(単品)で販売しているショップも見つかった場合は、
      こちらに別枠で掲載しています。<strong>2箱セットとは金額の単位が異なる</strong>
      ...
      },
      {
        ...
        totalLenses: 60,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデー 2箱セット(60枚・標準サイズ)",
        // 参考価格(2箱3,928円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 3100, max: 4900 },
        introHtml: `    <h2 class="section-heading">標準サイズ(30枚入り)の2箱でも比較したい方へ</h2>
    <p>
      90枚バリューパックとは別に、標準サイズ(30枚入り)を2箱で販売している
      ショップも見つかった場合は、こちらに別枠で掲載しています。
      ...
      },
      {
        ...
        totalLenses: 30,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデー 1箱(30枚・標準サイズ)",
        // 参考価格(1箱2,148円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 1700, max: 2700 },
        introHtml: `    <h2 class="section-heading">標準サイズ(30枚入り)の1箱でも比較したい方へ</h2>
    <p>
      「まずは1箱だけ試したい」という方向けに、標準サイズ(30枚入り)1箱の
      価格帯も別枠で掲載しています。<strong>90枚バリューパック・2箱セットとは
    ...
    <p>
      バイオトゥルーワンデーは、標準サイズ(30枚入り)を何箱まとめて購入するかや、
      まとめ買い向けの「90枚バリューパック」を選ぶかによって、1枚あたりの
      単価が変わります。当サイトでは、実勢価格でもっとも下がりやすい傾向のある
      単位を中心に比較しつつ、90枚バリューパック1箱・標準サイズ(30枚入り)の
      2箱・1箱で見つかった場合も、それぞれ別枠であわせて掲載しています。
    </p>
    <p class="note">
      ※ 販売単位や内容量は、時期やショップにより変更・終了している場合があります。
      ご購入前に各販売店の商品ページで最新の内容量・価格をご確認ください。
    ...
    productInfoHeading: "バイオトゥルーワンデーとは",
    productInfoHtml: `        <p>
          バイオトゥルーワンデーは、ボシュロムが展開する1日使い捨てタイプの
          コンタクトレンズです。生体模倣技術(バイオ・インスピレーション)から
          生まれたという独自素材「ハイパージェル」を採用し、レンズ含水率を
          ヒトの角膜と同じ水分量とされる78%まで高めているのが特徴とされています。
        </p>
        <h3>うるおいバリア機能によるうるおいキープ</h3>
        <p>
          涙が目のうるおいを保つ仕組みを応用したとされる設計により、レンズ表面に
          うるおい保持成分が結合し、水分の蒸発を抑えるバリア機能を果たすと
          されています。装用中のみずみずしさが続きやすいとされています。
        </p>
        <h3>非球面デザインとUVプロテクト機能</h3>
        <p>
          非球面レンズデザイン「HDオプティクス」により、夜間や薄暗い状況下での
          にじみ・ぼやけを軽減するとされています。あわせてUVプロテクト機能により
          UV-B波を97.8%、UV-A波を86.9%カットするとされています(UVカット機能は
          あくまで補助的なものであり、サングラス等の代わりにはなりませんので
          ご注意ください)。
        </p>
        <h3>こんな方におすすめ</h3>
  ...
  // ------------------------------------------------------------------
  {
    id: "biotrue-oneday-toric-saiyasu",
    slug: "biotrue-oneday-toric-saiyasu",
    outputDir: "docs-biotrue-oneday-toric",
    siteName: "バイオトゥルーワンデートーリック 乱視用最安値通販価格情報",
    theme: { accent: "#B8556B", gold: "#B8892B" }, // ローズ系(他18サイトと見分けやすい配色)
    historyUnitKey: "box6", // 価格推移グラフで記録する比較単位（6箱が実勢価格でも最安のため）
    searchKeyword: "バイオトゥルーワンデートーリック",
    metaDescription:
      "バイオトゥルーワンデートーリック 乱視用の楽天市場・Yahoo!ショッピングの価格を毎日チェックし、6箱・4箱・2箱・1箱それぞれの最安値トップ5を掲載しています。",
    subtitle:
      "「6箱」「4箱」「2箱」「1箱」それぞれの単位を基準に、楽天市場・Yahoo!ショッピングの価格を毎日チェックし、それぞれのショップ別最安値トップ5を掲載しています。",
    productSchemaName: "バイオトゥルーワンデートーリック 乱視用 6箱セット（180枚）",
    brandName: "Bausch+Lomb", // JSON-LD(構造化データ)のbrand.nameに使用
    brand: "ボシュロム", // トップページのブランド別グルーピングに使用
    brandKey: "bausch",
    shortName: "バイオトゥルーワンデートーリック 乱視用", // トップページの商品カード・フッターで使う短い正式名称
    ...
    /** 商品名が「バイオトゥルーワンデートーリック」であることを確認する */
    isCorrectProduct(name) {
      if (!name) return false;
      const n = name.replace(/\s/g, "");
      if (!/バイオトゥルー/.test(n)) return false;
      return /(トーリック|toric|乱視用)/i.test(n);
    },

    // 標準サイズ(30枚入り)の1/2/4/6箱で展開されている(90枚パック等の設定は無い)。
    units: [
      {
        key: "box6",
        label: "6箱(180枚)",
        totalLenses: 180,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデートーリック 乱視用 6箱セット(180枚)",
        // 参考価格(6箱14,780円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 12000, max: 18000 },
        introHtml: "",
        matches(name) {
          return isBoxCount(name, 6) || /180枚/.test(name.replace(/\s/g, ""));
      ...
      },
      {
        ...
        totalLenses: 120,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデートーリック 乱視用 4箱セット(120枚)",
        // 参考価格(4箱9,900円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 8000, max: 12000 },
        introHtml: `    <h2 class="section-heading">4箱(単品)でも比較したい方へ</h2>
    <p>
      4箱セット(120枚)で販売しているショップも見つかった場合は、
      ...
      },
      {
        ...
        totalLenses: 60,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデートーリック 乱視用 2箱セット(60枚)",
        // 参考価格(2箱5,060円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 4000, max: 6200 },
        introHtml: `    <h2 class="section-heading">2箱(単品)でも比較したい方へ</h2>
    <p>
      2箱セット(60枚)で販売しているショップも見つかった場合は、
      ...
      },
      {
        ...
        totalLenses: 30,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデートーリック 乱視用 1箱(30枚)",
        // 参考価格(1箱2,840円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 2200, max: 3500 },
        introHtml: `    <h2 class="section-heading">1箱(単品)でも比較したい方へ</h2>
    <p>
      「まずは1箱だけ試したい」という方向けに、1箱(30枚)の価格帯も
      別枠で掲載しています。<strong>6箱・4箱・2箱とは金額の単位が異なる</strong>
    ...
    productIntroHtml: `    <h2 class="section-heading">6箱(180枚)あたりの価格を中心に比較しています</h2>
    <p>
      バイオトゥルーワンデートーリック 乱視用は、1箱(30枚)を何箱まとめて
      購入するかによって、1枚あたりの単価が変わります。1枚あたりの単価が
      もっとも下がりやすいのは、6箱(180枚・約6ヶ月分)を購入した場合です。
      そのため当サイトでは、6箱あたりの実勢価格を中心に比較しつつ、
      4箱・2箱・1箱で見つかった場合も、それぞれ別枠であわせて掲載しています。
    </p>
    <p class="note">
    ...
    productInfoHeading: "バイオトゥルーワンデートーリック 乱視用とは",
    productInfoHtml: `        <p>
          バイオトゥルーワンデートーリック 乱視用は、ボシュロムが展開する
          1日使い捨てタイプの乱視矯正用コンタクトレンズです。近視用と
          同じ独自素材「ハイパージェル」を採用し、レンズ含水率78%による
          うるおいをキープする設計を継承しています。
        </p>
        <h3>オブリークプリズムデザインによる回転安定性</h3>
        <p>
          レンズ下部を薄くしたプリズムバラストデザインをさらに最適化した
          独自の「オブリークプリズムデザイン」により、4時〜8時方向の
          斜めのバラスト部分でレンズの回転を抑え、安定した乱視矯正を
          実現しているとされています(臨床試験では約97%が眼上での回転量
          10度以内と評価されています)。
        </p>
        <h3>非球面デザインによる見え方</h3>
        <p>
          非球面レンズデザイン「HDオプティクス」により、球面収差を補正し、
          夜間や薄暗い状況下でのにじみ・ぼやけを軽減するとされています。
        </p>
        <h3>こんな方におすすめ</h3>
        <ul>
          <li>乱視があり、装用中のうるおいを重視したい方</li>
  ...
  // ------------------------------------------------------------------
  // 19. バイオトゥルーワンデーマルチフォーカル
  // ------------------------------------------------------------------
  {
    id: "biotrue-oneday-multifocal-saiyasu",
    slug: "biotrue-oneday-multifocal-saiyasu",
    outputDir: "docs-biotrue-oneday-multifocal",
    siteName: "バイオトゥルーワンデーマルチフォーカル(遠近両用)最安値通販価格情報",
    theme: { accent: "#C97A3F", gold: "#B8892B" }, // アンバーオレンジ系(他19サイトと見分けやすい配色)
    historyUnitKey: "box6", // 価格推移グラフで記録する比較単位（6箱が実勢価格でも最安のため）
    searchKeyword: "バイオトゥルーワンデー マルチフォーカル",
    metaDescription:
      "バイオトゥルーワンデーマルチフォーカル(遠近両用)の楽天市場・Yahoo!ショッピングの価格を毎日チェックし、6箱・4箱・2箱・1箱それぞれの最安値トップ5を掲載しています。",
    subtitle:
      "「6箱」「4箱」「2箱」「1箱」それぞれの単位を基準に、楽天市場・Yahoo!ショッピングの価格を毎日チェックし、それぞれのショップ別最安値トップ5を掲載しています。",
    productSchemaName: "バイオトゥルーワンデーマルチフォーカル(遠近両用) 6箱セット（180枚）",
    brandName: "Bausch+Lomb", // JSON-LD(構造化データ)のbrand.nameに使用
    brand: "ボシュロム", // トップページのブランド別グルーピングに使用
    brandKey: "bausch",
    shortName: "バイオトゥルーワンデーマルチフォーカル(遠近両用)", // トップページの商品カード・フッターで使う短い正式名称
    ...
    specs: {
      ...
      water: 78, // 含水率(%)
      dk: 42, // 酸素透過率(Dk/t、参考値)
      uv: true, // UVカットの有無(UV-B97.8%/UV-A86.9%カットのUVプロテクト機能あり)
      origin: "海外",
      type: "遠近両用",
    },

    /** 商品名が「バイオトゥルーワンデーマルチフォーカル」であることを確認する */
    isCorrectProduct(name) {
      if (!name) return false;
      const n = name.replace(/\s/g, "");
      if (!/バイオトゥルー/.test(n)) return false;
      return /(マルチフォーカル|遠近両用)/i.test(n);
    },
    ...
    units: [
      {
        ...
        totalLenses: 180,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデーマルチフォーカル(遠近両用) 6箱セット(180枚)",
        // 参考価格(6箱16,680円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 13500, max: 20500 },
        introHtml: "",
        matches(name) {
          return isBoxCount(name, 6) || /180枚/.test(name.replace(/\s/g, ""));
        },
      ...
      },
      {
        ...
        totalLenses: 120,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデーマルチフォーカル(遠近両用) 4箱セット(120枚)",
        // 参考価格(4箱12,435円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 10000, max: 15000 },
        introHtml: `    <h2 class="section-heading">4箱(単品)でも比較したい方へ</h2>
    <p>
      4箱セット(120枚)で販売しているショップも見つかった場合は、
      こちらに別枠で掲載しています。<strong>6箱とは金額の単位が異なる</strong>ため、
      ...
      },
      {
        ...
        // 【試験導入→無効化済み】楽天の検索結果ページを直接読み取る機能
        // (enableSearchPageScrape)を試したが、検索結果ページ自体が
        // JavaScriptで後から商品を描画する作りのため、0件しか取得できず
        // 無効化した。同じ問題が起きた場合は、itemCode/pageUrl方式で
        // 個別に対応する。
        introHtml: `    <h2 class="section-heading">2箱(単品)でも比較したい方へ</h2>
    <p>
      2箱セット(60枚)で販売しているショップも見つかった場合は、
      ...
      },
      {
        ...
        totalLenses: 30,
        heroLabel: "本日の総合最安値（1枚あたり）",
        heroName: "バイオトゥルーワンデーマルチフォーカル(遠近両用) 1箱(30枚)",
        // 参考価格(1箱3,220円)をもとに、価格帯を直接指定して追加取得する
        priceHint: { min: 2600, max: 4000 },
        introHtml: `    <h2 class="section-heading">1箱(単品)でも比較したい方へ</h2>
    <p>
      「まずは1箱だけ試したい」という方向けに、1箱(30枚)の価格帯も
    ...
    productIntroHtml: `    <h2 class="section-heading">6箱(180枚)あたりの価格を中心に比較しています</h2>
    <p>
      バイオトゥルーワンデーマルチフォーカル(遠近両用)は、1箱(30枚)を
      何箱まとめて購入するかによって、1枚あたりの単価が変わります。
      1枚あたりの単価がもっとも下がりやすいのは、6箱(180枚・約6ヶ月分)を
      購入した場合です。そのため当サイトでは、6箱あたりの実勢価格を
      中心に比較しつつ、4箱・2箱・1箱で見つかった場合も、それぞれ別枠で
      あわせて掲載しています。
    </p>
    <p class="note">
    ...
    productInfoHeading: "バイオトゥルーワンデーマルチフォーカル(遠近両用)とは",
    productInfoHtml: `        <p>
          バイオトゥルーワンデーマルチフォーカル(遠近両用)は、ボシュロムが
          展開する1日使い捨てタイプの遠近両用コンタクトレンズです。
          通常版と同じ独自素材「ハイパージェル」を採用し、角膜と同等の
          含水率(78%)によるうるおいをキープする設計を継承しています。
        </p>
        <h3>スリーゾーン累進デザイン</h3>
        <p>
          手元がくっきり見える「近用部」、パソコンなど中間距離が見やすい
          「中間部」、遠くの景色までクリアに見られる「遠用部」の3つの
          ゾーンをなめらかに移動できる設計とされています。加入度数は
          HIGH(MAX+2.50)・LOW(MAX+1.50)の2種類の展開です。
        </p>
        <h3>高い酸素透過率とUVプロテクト機能</h3>
        <p>
          裸眼時の93%の酸素を角膜に供給するとされる高い酸素透過率を
          実現しています。あわせてUVプロテクト機能により、UV-B波を97.8%、
          UV-A波を86.9%カットするとされています(UVカット機能はあくまで
          補助的なものであり、サングラス等の代わりにはなりませんので
          ご注意ください)。
        </p>
        <h3>こんな方におすすめ</h3>
        <ul>
  ...
  // ------------------------------------------------------------------
  // 20. 新しい商品を追加する場合は、ここに同じ形式でオブジェクトを追加する。
  // ------------------------------------------------------------------
];
